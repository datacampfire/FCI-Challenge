...
PP0009 = read_delim("~/datacampfire/FCI-Challenge/datasets_originales/PP0009.csv", ";", escape_double = FALSE, trim_ws = TRUE, col_names = FALSE, skip = 1) %>% 
  ...
  mutate_all(funs(gsub("\xb0" , "\u00b0", .))) %>% 
  mutate(anio_informado = as.integer(anio_informado)) %>%  # fix numbers
  mutate_at(.cols = vars(enero,febrero,marzo,abril,mayo,junio,julio,agosto,septiembre,octubre,noviembre,diciembre),
...
write_csv(PP0008, "datasets_corregidos/PP0008.csv")
write_csv(PP0009, "datasets_corregidos/PP0009.csv")
write_csv(PP0010, "datasets_corregidos/PP0010.csv")
write_csv(PP0011, "datasets_corregidos/PP0011.csv")
write_csv(PP0012, "datasets_corregidos/PP0012.csv")
write_csv(PP0013, "datasets_corregidos/PP0013.csv")
write_csv(PP0014, "datasets_corregidos/PP0014.csv")
write_csv(PP0021, "datasets_corregidos/PP0021.csv")
