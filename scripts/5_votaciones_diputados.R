...
dir.create("diputados")
# file.remove(dir("diputados/", full.names = TRUE))
...
get_vote_data <- function(id = 25886) { # id <- sample(ids, size = 1)
  ...
  fout <- sprintf("diputados/votacion_%s.rds", id)
...
files <- dir("diputados/", full.names = TRUE, pattern = "votacion_\\d+.rds")
...
# export ------------------------------------------------------------------
write_csv(data_votacion_detalle, "diputados/votacion_detalle.csv")
write_csv(data_diputados, "diputados/diputados.csv")
write_csv(data_votacion_meta, "diputados/votacion_metadata.csv")
